...
const getSinglePoolSwapData = async (poolId, poolNum=1, outOf=1, historical = false, timeInterval = null) => {
    ...
    while (timestamp_high >= lowerBound) {
        while (moreResults) {
            ...
            // console.log('Calling pair: (poolId, high, low): ' + '('+ poolId +',' +timestamp_high +','+ timestamp_low  +') - got (amount results)' + '(' + res.data.data.swaps.length + ')' );
...
const getSwapDataByTimeInterval = async (pool, historical = false) => {
    ...
    // TODO Check Rate
    data.forEach(e => e.rate = ( liquidityInRange.getPriceForTick(e.tick) ));
    const res = {};
